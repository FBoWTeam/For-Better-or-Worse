    ...
    {   
		switch (mode)
		{
			...
			case "player1":
				...
				StartCoroutine(GameManager.gameManager.UIManager.PowerCooldownSystem(true, power, cooldown));
				yield return new WaitForSeconds(cooldown);
				canBeActivatedByPlayer1[(int)power - 1] = true;
				break;
			case "player2":
				canBeActivatedByPlayer2[(int)power - 1] = false;
				StartCoroutine(GameManager.gameManager.UIManager.PowerCooldownSystem(false, power, cooldown));
				yield return new WaitForSeconds(cooldown);
